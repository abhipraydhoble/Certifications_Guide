# AWS Certification Roadmap 📘

<img width="1542" height="514" alt="image" src="https://github.com/user-attachments/assets/ea351e3f-a176-4632-9976-2ef3d2ba8423" />



---

## 🧱 AWS Certification Levels

AWS certifications are divided into **four levels**:

1. **Foundational**
2. **Associate**
3. **Professional**
4. **Specialty**

Each level builds on the previous one.

---

## 1️⃣ Foundational Level

## AWS Cloud Practitioner (CLF-C02)

### 🎯 Focus

* Understand **what AWS services do**
* Learn **basic cloud concepts**
* Introduction to **security and billing**

### 🔹 Compute

* Amazon EC2
* AWS Lambda
* Amazon Lightsail

### 🔹 Storage

* Amazon S3
* Amazon EBS

### 🔹 Networking

* Amazon VPC
* Amazon Route 53

### 🔹 Databases

* Amazon RDS
* Amazon DynamoDB

### 🔹 Security & Governance

* AWS IAM
* AWS Trusted Advisor
* AWS Shield

### 🔹 Billing & Cost Management

* AWS Cost Explorer
* AWS Budgets

---

## 2️⃣ Associate Level

## AWS Solutions Architect – Associate (SAA-C03)

### 🎯 Focus

* **Designing AWS architectures**
* Understanding **how services connect**
* High availability, scalability, and fault tolerance

### 🔹 High Availability

* Elastic Load Balancer (ELB)
* AWS Auto Scaling

### 🔹 Decoupling & Messaging

* Amazon SQS (Queues)
* Amazon SNS (Notifications)
* Amazon EventBridge

### 🔹 Advanced Storage

* Amazon EFS
* AWS Storage Gateway
* Amazon FSx

### 🔹 Databases

* Amazon Aurora
* Amazon ElastiCache
* Amazon Redshift

### 🔹 Security

* AWS KMS (Encryption)
* AWS Secrets Manager

---

## 3️⃣ Professional Level

## AWS Solutions Architect – Professional (SAP-C02)

### 🎯 Focus

* **Complex, enterprise-scale architectures**
* Multi-account environments
* Automation, governance, and migrations

### 🔹 Hybrid & Networking

* AWS Direct Connect
* AWS Transit Gateway
* VPC Peering

### 🔹 Governance & Management

* AWS Organizations
* AWS Control Tower
* AWS Service Catalog

### 🔹 Migration Services

* AWS Application Migration Service (MGN)
* AWS Database Migration Service (DMS)

### 🔹 Data Analytics

* AWS Glue
* Amazon Kinesis
* Amazon Athena

### 🔹 Automation

* AWS CloudFormation
* StackSets (Multi-region & Multi-account)

---
## 4️⃣ Specialty Level

### AWS Specialty Certifications

**Focus:**

* Deep expertise in **specific domains**
* Advanced troubleshooting & design

### 🎯 Available Specialty Certifications

* **AWS Security – Specialty (SCS-C02)**
* **AWS Advanced Networking – Specialty (ANS-C01)**
* **AWS Machine Learning – Specialty (MLS-C01)**
* **AWS Data Analytics – Specialty (DAS-C01)**
---
## 💰 AWS Exam Cost (India – Approximate)

| Certification Level | Base Fee | Total with 18% GST |
| ------------------- | -------- | ------------------ |
| Cloud Practitioner  | $100     | ~₹10,000           |
| Associate           | $150     | ~₹15,000           |
| Professional        | $300     | ~₹30,000           |

---



## 📚 Important Documents & Learning Links (Official + Recommended)

---

## 1️⃣ AWS Cloud Practitioner (CLF-C02)

### ✅ Official AWS Resources

* **Official Exam Guide**
  [https://aws.amazon.com/certification/certified-cloud-practitioner/](https://aws.amazon.com/certification/certified-cloud-practitioner/)

* **Exam Guide (PDF)**
  [https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf](https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf)

* **Official Sample Questions**
  [https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Sample-Questions.pdf](https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Sample-Questions.pdf)

* **AWS Cloud Practitioner Essentials (FREE – AWS Skill Builder)**
  [https://explore.skillbuilder.aws/learn/course/external/view/elearning/134/aws-cloud-practitioner-essentials](https://explore.skillbuilder.aws/learn/course/external/view/elearning/134/aws-cloud-practitioner-essentials)

---

## 2️⃣ AWS Solutions Architect – Associate (SAA-C03)

### ✅ Official AWS Resources

* **Official Certification Page**
  [https://aws.amazon.com/certification/certified-solutions-architect-associate/](https://aws.amazon.com/certification/certified-solutions-architect-associate/)

* **Exam Guide (PDF)**
  [https://d1.awsstatic.com/training-and-certification/docs-sa-assoc/AWS-Certified-Solutions-Architect-Associate_Exam-Guide.pdf](https://d1.awsstatic.com/training-and-certification/docs-sa-assoc/AWS-Certified-Solutions-Architect-Associate_Exam-Guide.pdf)

* **Official Sample Questions**
  [https://d1.awsstatic.com/training-and-certification/docs-sa-assoc/AWS-Certified-Solutions-Architect-Associate_Sample-Questions.pdf](https://d1.awsstatic.com/training-and-certification/docs-sa-assoc/AWS-Certified-Solutions-Architect-Associate_Sample-Questions.pdf)

* **AWS Skill Builder – Exam Prep (Mostly Free)**
  [https://explore.skillbuilder.aws/learn/course/external/view/elearning/140/aws-certified-solutions-architect-associate-exam-prep](https://explore.skillbuilder.aws/learn/course/external/view/elearning/140/aws-certified-solutions-architect-associate-exam-prep)

### 📘 Architecture Must-Read Docs

* **AWS Well-Architected Framework (VERY IMPORTANT)**
  [https://aws.amazon.com/architecture/well-architected/](https://aws.amazon.com/architecture/well-architected/)

* **AWS Reference Architectures**
  [https://aws.amazon.com/architecture/](https://aws.amazon.com/architecture/)

* **AWS Whitepapers & Guides**
  [https://aws.amazon.com/whitepapers/](https://aws.amazon.com/whitepapers/)

---

## 3️⃣ AWS Solutions Architect – Professional (SAP-C02)

### ✅ Official AWS Resources

* **Official Certification Page**
  [https://aws.amazon.com/certification/certified-solutions-architect-professional/](https://aws.amazon.com/certification/certified-solutions-architect-professional/)

* **Exam Guide (PDF)**
  [https://d1.awsstatic.com/training-and-certification/docs-sa-pro/AWS-Certified-Solutions-Architect-Professional_Exam-Guide.pdf](https://d1.awsstatic.com/training-and-certification/docs-sa-pro/AWS-Certified-Solutions-Architect-Professional_Exam-Guide.pdf)

* **Official Sample Questions**
  [https://d1.awsstatic.com/training-and-certification/docs-sa-pro/AWS-Certified-Solutions-Architect-Professional_Sample-Questions.pdf](https://d1.awsstatic.com/training-and-certification/docs-sa-pro/AWS-Certified-Solutions-Architect-Professional_Sample-Questions.pdf)

* **AWS Skill Builder – Exam Readiness**
  [https://explore.skillbuilder.aws/learn/course/external/view/elearning/142/aws-certified-solutions-architect-professional-exam-readiness](https://explore.skillbuilder.aws/learn/course/external/view/elearning/142/aws-certified-solutions-architect-professional-exam-readiness)

---

## ⭐ Highly Recommended (All Levels)

### 🔹 AWS Core Documentation (Daily Use)

* AWS Documentation Home
  [https://docs.aws.amazon.com/](https://docs.aws.amazon.com/)

* AWS Pricing Calculator
  [https://calculator.aws/](https://calculator.aws/)

* AWS Free Tier
  [https://aws.amazon.com/free/](https://aws.amazon.com/free/)

---

### 🔹 Hands-On & Practice

* **AWS Workshops (FREE Hands-On Labs)**
  [https://workshops.aws/](https://workshops.aws/)

* **AWS Samples (GitHub – Real Examples)**
  [https://github.com/aws-samples](https://github.com/aws-samples)

---

### 🔹 Exam & Interview Preparation

* **AWS FAQs (Hidden Exam Gold)**
  [https://aws.amazon.com/faqs/](https://aws.amazon.com/faqs/)

* **AWS Re:Invent Videos (Deep Concepts)**
  [https://www.youtube.com/@AWSEvents](https://www.youtube.com/@AWSEvents)

---




